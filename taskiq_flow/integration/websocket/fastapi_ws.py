...
class FastAPIWebSocketManager:
    ...
    async def broadcast_event(self, channel: str, event: dict[str, Any]) -> None:
        ...
        """
        # Track metrics before broadcast
        parts = channel.split(".")
        pipeline_id = parts[1] if len(parts) > 1 else channel
...
async def fastapi_websocket_endpoint(
    ...
    try:
        ...
        MetricsCollector().websocket_message(pipeline_id, "in", "connection")
        ...
        logger.info("Client disconnected from pipeline %s", pipeline_id)
    except Exception as e:
        logger.error("WebSocket error for pipeline %s: %s", pipeline_id, e)
    finally:
