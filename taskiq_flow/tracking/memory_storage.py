...
class InMemoryPipelineStorage(PipelineStorage):
    ...
    async def start_pipeline(self, pipeline_id: str) -> None:
        ...
        async with self._lock:
            if pipeline_id in self._pipelines:
                ...
                self._pipelines[pipeline_id].started_at = datetime.now(timezone.utc)
    ...
    async def complete_pipeline(self, pipeline_id: str, result: Any) -> None:
        ...
        async with self._lock:
            if pipeline_id in self._pipelines:
                ...
                self._pipelines[pipeline_id].finished_at = datetime.now(timezone.utc)
    ...
    async def start_step(
        ...
        async with self._lock:
            if pipeline_id in self._pipelines and step_index < len(
