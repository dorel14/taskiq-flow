...
class PersistenceAdapter(BaseStorageAdapter):
    ...
    async def _create_tables_async(self, db_url: str) -> None:
        ...
        sync_url = db_url.replace("asyncpg", "psycopg2").replace("aiosqlite", "sqlite")
    ...
    async def set(
        ...
        if isinstance(value, dict) and value.get("model_type") == "SchedulerJobModel":
    ...
    async def load_jobs(self) -> list[SchedulerJob]:
        ...
        return [
            SchedulerJob(
                ...
                interval_seconds=(
                    int(j.interval_seconds) if j.interval_seconds is not None else None
                ),
