...
def read_uvlock_version() -> str | None:
...
def write_uvlock_version(new_version: str) -> None:
    ...
    uvlock = PROJECT_ROOT / "uv.lock"
    if not uvlock.exists():
        logger.info("⚠️  uv.lock not found, skipping")
        return
    content = uvlock.read_text(encoding="utf-8")
    ...
    if new_content == content:
        logger.info("⚠️  Could not find taskiq-flow version in uv.lock,\
...
def check_consistency() -> bool:
    ...
    logger.info(f"📦 pyproject.toml version: {pyproject_version}")
    if uvlock_version:
        logger.info(f"📦 uv.lock version: {uvlock_version}")
    else:
        logger.info("📦 uv.lock: not found or no version")

    consistent = True
    if uvlock_version and uvlock_version != pyproject_version:
        logger.info("❌ Version mismatch between pyproject.toml and uv.lock!")
        consistent = False
    else:
        logger.info("✅ Versions are consistent")
    ...
    try:
        ...
        if args.bump:
            ...
            if not re.match(r"^\d+\.\d+\.\d+$", args.set):
                logger.info("❌ Version must be in X.Y.Z format")
                sys.exit(1)
            new_version = args.set

        if new_version == current_version:
            logger.info("Version unchanged")
            sys.exit(0)
        ...
        logger.info(f'   git commit -m "bump: version {new_version}"')
        logger.info(f'   git tag -a v{new_version} -m "Release v{new_version}"')
        logger.info("   git push origin develop --follow-tags")
